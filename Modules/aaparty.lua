...
local function MessageHandler()
    aaActor = MyUI_Actor.register('aa_party', function(message)
        ...
        if subject == 'Hello' then
            ...
            aaActor:send({ mailbox = 'aa_party', script = 'grimgui', }, GenerateContent(nil, 'Welcome'))
...
local function getMyAA()
    ...
    if not changed and CheckIn() then
        aaActor:send({ mailbox = 'aa_party', script = 'grimgui', }, GenerateContent(nil, 'CheckIn'))
    ...
    if changed then
        ...
        aaActor:send({ mailbox = 'aa_party', script = 'grimgui', }, GenerateContent())
        checkIn = os.time()
        changed = false
    end
end
...
local function SayGoodBye()
    ...
    aaActor:send({ mailbox = 'aa_party', script = 'grimgui', }, message)
end
...
local function DrawStatusIcon(iconID, type, txt)
    ...
    if type == 'item' then
        ImGui.DrawTextureAnimation(MyUI_Utils.Animation_Item, iconSize, iconSize)
    elseif type == 'pwcs' then
        ...
        ImGui.DrawTextureAnimation(MyUI_Utils.Animation_Spell, iconSize, iconSize)
    end
                    ...
                    if groupData[i] ~= nil then
                        ...
                        if expand[groupData[i].Name] then
                            ...
                            if imgui.Button("<##Decrease" .. groupData[i].Name) then
                                ...
                                aaActor:send({ mailbox = 'aa_party', script = 'grimgui', }, GenerateContent(groupData[i].Name, 'Action', 'Less'))
                            end
                            imgui.SameLine()
                            ...
                            if imgui.Button(">##Increase" .. groupData[i].Name) then
                                aaActor:send({ mailbox = 'aa_party', script = 'grimgui', }, GenerateContent(groupData[i].Name, 'Action', 'More'))
...
local function init()
    ...
    aaActor:send({ mailbox = 'aa_party', script = 'grimgui', }, GenerateContent(nil, 'Hello'))
end
