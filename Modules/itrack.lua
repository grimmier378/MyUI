...
if not loadedExternally then
    ...
    Module.ThemeName   = MyUI.ThemeName or 'Default'
    Module.Theme       = MyUI.Theme or {}
end
...
local function sortTrackedItems()
    ...
    for _, item in ipairs(tmp) do
        local exists = false
        for _, filteredItem in ipairs(filteredTmp) do
            if filteredItem.name:lower() == item.name:lower() then
                exists = true
                break
            end
        end
        if not exists then
            table.insert(filteredTmp, item)
        end
    end

    table.sort(filteredTmp, function(a, b) return a.name < b.name end)
    trackedItems = {}
    for _, item in ipairs(filteredTmp) do
