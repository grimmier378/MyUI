            ...
            if showMain then
                ...
                if petName == 'NO PET' then
                    ...
                    if ImGui.BeginTable("##SplitWindow", 2, bit32.bor(ImGuiTableFlags.BordersOuter, ImGuiTableFlags.Resizable, ImGuiTableFlags.Reorderable, ImGuiTableFlags.Hideable), ImVec2(-1, -1)) then
                        ...
                        ImGui.BeginGroup()
                        Module.Utils.DropShadow("Lvl:", { Enabled = true, })
                        -- ImGui.Text("Lvl:")
                        ImGui.SameLine()
                        Module.Utils.DropShadow(string.format("%s", petLvl), { Enabled = true, }, nil, Module.Colors.color('teal'))
                        -- ImGui.TextColored((Module.Colors.color('teal')), "%s", petLvl)
                        ImGui.SameLine()
                        Module.Utils.DropShadow("Dist:", { Enabled = true, })
                        ImGui.SameLine()
                        petDist = myPet.Distance() or 0

                        if petDist >= 150 then
                            Module.Utils.DropShadow(string.format("%.0f", petDist), { Enabled = true, }, nil, Module.Colors.color('red'))
                        else
                            Module.Utils.DropShadow(string.format("%.0f", petDist), { Enabled = true, }, nil, Module.Colors.color('green'))
                        end
                        ...
                        -- Module.Utils.DropShadow(string.format("%.1f%%", petHP), { Enabled = true, })
                        ImGui.EndGroup()
                        if ImGui.IsItemHovered() then
                        ...
                        Module.Utils.DropShadow(string.format("%s", petTarg), { Enabled = true, }, nil, ImVec4(txCol[1], txCol[2], txCol[3], txCol[4]))
    ...
    if showConfigGUI then
        ...
        if showConfig then
            ...
            if ImGui.CollapsingHeader('Buttons##PetConfigButtons') then
                ...
                local sorted_names = MyUI.Utils.SortTableColumns(nil, btnKeys, col)
...
local function Init()
    ...
    if not loadedExeternally then
        ...
        petBuffs = MyUI.MyPetData.Buffs
        petBuffCount = MyUI.MyPetData.BuffCount
        btnInfo = MyUI.MyPetData.ButtonStates
    end
end
...
function Module.MainLoop()
    if loadedExeternally then
        ...
        petBuffCount = MyUI.MyPetData.BuffCount or 0
        btnInfo = MyUI.MyPetData.ButtonStates or {}
        petName = MyUI.MyPetData.Name or 'NO PET'
    end
